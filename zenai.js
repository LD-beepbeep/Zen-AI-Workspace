...
const CARET = {
  ...
  update() {
    ...
    if (!rect.height) { this.el.style.display = 'none'; return; }

    this.el.style.display = 'block';
    this.el.style.transition = this.smooth
      ? 'top 0.06s cubic-bezier(0.2,0,0,1), left 0.06s cubic-bezier(0.2,0,0,1)'
      : 'none';
    ...
    if (sh === 'underline') {
      ...
      this.el.style.width  = (rect.width > 2 ? rect.width : 9) + 'px';
    } else {
      this.el.style.height = rect.height + 'px';
...
function onEdInput()    { updateStats(); if (S.typewriter) doTypewriter(); }
...
function onPaste()    { requestAnimationFrame(() => { updateStats(); CARET.update(); }); }
function pastePlain() {
...
function exitFocus() {
  ...
  document.getElementById('fed').innerHTML = ''; _foc = false; updateStats();
}
function onFedInput() { updateStats(); }
...
function toggleAI() {
  _aiOpen = !_aiOpen;
  document.getElementById('aisb').style.display = _aiOpen ? 'flex' : 'none';
  document.getElementById('aibtn').style.color  = _aiOpen ? 'var(--acc)' : 'var(--tx3)';
  updateApiUI();
}
...
function openSettings() {
  ...
  const sync = (id, val, fmt = v=>v) => { const el=document.getElementById(id); if(el) el.value=val; };
  sync('fs-r', S.fontSize);   document.getElementById('fs-val').textContent  = S.fontSize+'px';
  sync('lh-r', S.lineHeight); document.getElementById('lh-val').textContent  = S.lineHeight.toFixed(2);
  sync('ew-r', S.editorWidth);document.getElementById('ew-val').textContent  = S.editorWidth+'px';
  sync('pg-r', S.paraSpacing);pvPG(S.paraSpacing);
  sync('cw-r', S.caretWidth); document.getElementById('cw-val').textContent  = S.caretWidth+'px';
  sync('bs-r', S.blinkSpeed); document.getElementById('bs-val').textContent  = S.blinkSpeed.toFixed(2)+'s';
  sync('vol-r',SFX.volume);   document.getElementById('vol-val').textContent = Math.round(SFX.volume*100)+'%';
...
function showStab(name, btn) {
  ...
  document.querySelectorAll('[id^="sp-"]').forEach(p => { p.style.display='none'; p.classList.remove('on'); });
  btn.classList.add('on');
  const panel = document.getElementById('sp-'+name);
  if (panel) { panel.style.display='flex'; panel.style.flexDirection='column'; panel.classList.add('on'); }
}
